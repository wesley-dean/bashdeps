...
This ADR defines how consuming projects obtain `bashdeps` itself and how Make
...
recipes after `bashdeps` is available.
...
- Keep `bashdeps` itself outside dependency destinations managed by the consumer
  manifest.
...
A consuming project SHOULD place its bootstrapped `bashdeps` artifact outside the
paths managed by `dependencies.txt`.
...
and committed SHA-256 digest.  The bootstrap recipe SHALL:
...
the consumer because bashdeps is not yet available to implement that first step.
...
```text
make deps        bootstrap bashdeps if necessary, then bashdeps sync
make deps-check  run bashdeps verify using an already-present bootstrap artifact
...
`make deps-check` SHALL NOT bootstrap a missing `bashdeps` artifact.  If the
...
`deps-check` SHALL invoke `bashdeps verify` and therefore remain network-free and
non-mutating with respect to managed dependencies.

`make build` SHALL NOT implicitly invoke `bashdeps sync`, `bashdeps verify`, or
the bootstrap recipe.  A build may fail because required inputs are absent; that
failure is preferable to an ordinary build unexpectedly changing repository
state or requiring network access.
...
### Put bashdeps in dependencies.txt

This creates a circular dependency: bashdeps would be required to materialize the
manifest entry that provides bashdeps.  The bootstrap artifact therefore remains
outside the managed manifest.
...
### Vendor bashdeps permanently in every consumer repository
...
independent trust required for the first bashdeps artifact.
