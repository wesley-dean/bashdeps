...
## @brief Whether the parsed update CLI requested whole-manifest `--all`
## behavior.
__manifest_manager_option_all=0

## @var __manifest_manager_option_package
## @brief Single package selected by the parsed update CLI when not using
## `--all`.
...
## @brief Whether target version selection is explicit rather than
## latest-release.
...
## @brief Verifies that one record is release-updateable before network
## activity.
## @details
...
## Using the current version as a provisional target exercises URL recognition
## without selecting or retrieving a new release.
...
## @brief Selects every record only after complete non-network eligibility
## checks.
## @details
## `--all` means every dependency.  The helper therefore validates every
## record's manager identity, rejects commit pins and unsupported URL forms, and
## requires package names to be unique before curl or hashing capability is
## consulted. Unsupported dependencies are never silently skipped.
## @par STDIN
## Nothing is read from STDIN.
## @par STDOUT
## Nothing is written to STDOUT.
...
## @details
## The helper derives the candidate URL, retrieves exact candidate bytes, hashes
## them, rejects changed bytes at an unchanged immutable URL, and applies
## literal field-value substitutions only when each changed old value occurs
## exactly once in the original raw record.  `dest` is never changed.  The
...
## Nothing is written to STDOUT.
## @par STDERR
## Diagnostics identify URL, acquisition, hashing, or surgical-safety failures.
## @returns Nothing is written to STDOUT.
...
## @retval 5 Exact substitution or immutable-location safety failed.
...
__manifest_manager_plan_record_update() {
  ...
  local __mm_artifact __mm_hash __mm_new_digest __mm_new_id
...
## dependency needs planning.  All records are fully planned before the
## candidate is emitted and preservation-proved.
...
## @retval 3 A required runtime capability is unavailable.
## @retval 4 Release discovery or candidate acquisition failed.
## @retval 5 Surgical or immutable-location safety validation failed.
...
## Nothing is written to STDOUT.
## @par STDERR
## A diagnostic identifies invalid, missing, duplicate, or conflicting
## arguments.
## @returns Nothing is written to STDOUT.
